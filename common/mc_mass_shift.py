        ...
        for ptbin in zip(PT_BINS[:-1], PT_BINS[1:]):
            for ctbin in zip(CT_BINS[:-1], CT_BINS[1:]):
                ...
                for eff, tsd in zip(pd.unique(eff_score_array[0][::-1]), pd.unique(eff_score_array[1][::-1])):
                    ...
                    mean_shift.SetBinError(shift_bin,iEff,math.sqrt(h1_sel.GetMeanError()*h1_sel.GetMeanError())*1000)
                    ...
                    fit_shift.SetBinError(shift_bin,iEff,math.sqrt(err_mu_sel*err_mu_sel)*1000)
                    ...
                    if round(eff,2)==round(SEL_EFF[eff_index],2):
                        ...
                        opt_shift.SetBinError(shift_bin,math.sqrt(h1_sel.GetMeanError()*h1_sel.GetMeanError())*1000)
                      
                        opt_fit_shift.SetBinContent(shift_bin,(mu_sel-hyp3mass)*1000)
                        opt_fit_shift.SetBinError(shift_bin,math.sqrt(err_mu_sel*err_mu_sel)*1000)
